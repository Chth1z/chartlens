...
def chatgpt_token_cache_status() -> dict[str, Any]:
    path = _cache_path()
    cache = load_chatgpt_tokens()
    tokens = (cache or {}).get("tokens") if isinstance((cache or {}).get("tokens"), dict) else {}
    user = (cache or {}).get("user") if isinstance((cache or {}).get("user"), dict) else None
    expires_at = tokens.get("expires_at") if isinstance(tokens, dict) else None
    if expires_at is None and isinstance(tokens, dict):
        expires_at = _jwt_exp(str(tokens.get("access_token") or ""))
    return {
        "token_cache_exists": path.exists(),
        "token_cache_path": str(path),
        "updated_at": (cache or {}).get("updated_at") if cache else None,
