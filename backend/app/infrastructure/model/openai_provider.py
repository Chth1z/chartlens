...
class OpenAIResponsesProvider(ModelProvider):
    ...
    def __init__(self, *, mode: str = "standard") -> None:
        if not settings.openai_api_key:
            raise RuntimeError("CHARTLENS_OPENAI_API_KEY is required for OpenAIResponsesProvider")
    ...
    def extract_fields(
        ...
        *,
        ...
        return _field_results_from_payload(parsed["results"], evidence_by_field)
    ...
    *,
    ...
    user_payload = {
        "rules": [
            ...
            "Return evidence_id from the evidence item you used when available.",
        ],
        ...
        "evidence_by_field": {
            key: [
                _candidate_prompt_item(key, index, candidate, max_text_chars=320)
                for index, candidate in enumerate(candidates[: _max_evidence_items_for_key(key, fields)])
            ]
            for key, candidates in evidence_by_field.items()
            if key != "__case_context__"
        },
        "case_context": [
            _candidate_prompt_item("__case_context__", index, candidate, max_text_chars=80)
            for index, candidate in enumerate(evidence_by_field.get("__case_context__", []))
        ],
    }
    payload = {
        "model": model,
...
def _candidate_prompt_item(field_key: str, index: int, candidate: EvidenceCandidate, *, max_text_chars: int) -> dict[str, Any]:
    return {
        "id": _evidence_id(field_key, index),
        "text": _truncate_text(candidate.text, max_text_chars),
        "page": candidate.page,
...
def _max_evidence_items_for_key(field_key: str, fields: list[FieldDefinition]) -> int:
    for field in fields:
        if field.key == field_key:
            return field.llm.max_evidence_items_for_llm
    return 2
...
def _evidence_id(field_key: str, index: int) -> str:
    return f"{field_key}:{index + 1}"


def _field_results_from_payload(
    items: list[dict[str, Any]],
    evidence_by_field: dict[str, list[EvidenceCandidate]],
) -> list[FieldExtractionResult]:
    lookup = _evidence_lookup(evidence_by_field)
    results: list[FieldExtractionResult] = []
    for item in items:
        payload = dict(item)
        evidence_id = payload.pop("evidence_id", None)
        evidence = lookup.get(evidence_id) if isinstance(evidence_id, str) else None
        if evidence is not None:
            payload["evidence_text"] = payload.get("evidence_text") or evidence.text
            payload["page"] = payload.get("page") or evidence.page
            payload["bbox"] = payload.get("bbox") or evidence.bbox
        results.append(FieldExtractionResult.model_validate(payload))
    return results


def _evidence_lookup(evidence_by_field: dict[str, list[EvidenceCandidate]]) -> dict[str, EvidenceCandidate]:
    lookup: dict[str, EvidenceCandidate] = {}
    for field_key, candidates in evidence_by_field.items():
        for index, candidate in enumerate(candidates):
            lookup[_evidence_id(field_key, index)] = candidate
    return lookup
