...
def ocr_file(
    ...
    *,
    ...
    if suffix in IMAGE_SUFFIXES:
        return _blocks_from_images([path], engine=engine, ocr_profile=profile, cache_namespace=cache_namespace, cache_stats=cache_stats)
...
def _blocks_from_images(
    ...
    *,
    ...
    stats.setdefault("page_cache_hit_count", 0)
    stats.setdefault("page_cache_miss_count", 0)
    stats.setdefault("second_pass_ocr_pages", [])
    ...
    if engine is not None or max_workers <= 1 or len(image_paths) <= 1:
        ...
        for page, image_path in enumerate(image_paths, start=1):
            ...
            page_blocks = _extract_page_blocks_with_cache(
                page,
                input_path,
                profile,
                ocr_engine,
        ...
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="chartlens-ocr-page") as executor:
            futures = [
...
def _extract_page_blocks_with_cache(
    page: int,
    image_path: Path,
    ...
    *,
    ...
    key = _page_cache_key(image_path, page, profile, cache_namespace)
    if key is not None:
        cached = _load_cached_page_blocks(key)
        if cached is not None:
            cache_stats["page_cache_hit_count"] = cache_stats.get("page_cache_hit_count", 0) + 1
            return cached
    page_blocks = engine.extract_blocks(image_path, page)
    second_pass_blocks = _second_pass_if_low_confidence(page, image_path, profile, engine, page_blocks)
    if second_pass_blocks is not page_blocks:
        second_pass_pages = cache_stats.setdefault("second_pass_ocr_pages", [])
        if isinstance(second_pass_pages, list):
            second_pass_pages.append(page)
        page_blocks = second_pass_blocks
    if key is not None:
        cache_stats["page_cache_miss_count"] = cache_stats.get("page_cache_miss_count", 0) + 1
        _save_cached_page_blocks(key, page_blocks)
    return page_blocks


def _second_pass_if_low_confidence(
    page: int,
    image_path: Path,
    profile: OcrProfileConfig,
    engine: ImageOcrEngine,
    ...
    if not isinstance(engine, LocalOcrEngine) or not page_blocks:
        return page_blocks
    avg_confidence = sum(block.confidence for block in page_blocks) / len(page_blocks)
    if avg_confidence >= profile.low_confidence_threshold or len(profile.engine_priority) < 2:
        return page_blocks
    alternate_priority = [*profile.engine_priority[1:], profile.engine_priority[0]]
    alternate_blocks = LocalOcrEngine(alternate_priority).extract_blocks(image_path, page)
    if not alternate_blocks:
        return page_blocks
    alternate_confidence = sum(block.confidence for block in alternate_blocks) / len(alternate_blocks)
    return alternate_blocks if alternate_confidence > avg_confidence else page_blocks
...
def _page_cache_key(
    ...
    payload = {
        ...
        "pdf_dpi": profile.pdf_dpi,
        "preprocess": profile.preprocess.model_dump(),
        "engine_priority": profile.engine_priority,
    }
    return hashlib.sha256(json.dumps(payload, sort_keys=True).encode("utf-8")).hexdigest()


def _load_cached_page_blocks(key: str) -> list[OcrBlock] | None:
    path = _page_cache_path(key)
    if not path.exists():
        return None
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
        return [OcrBlock.model_validate(item) for item in payload.get("blocks", [])]
    except (OSError, ValueError, json.JSONDecodeError):
        return None


def _save_cached_page_blocks(key: str, blocks: list[OcrBlock]) -> None:
    path = _page_cache_path(key)
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = path.with_suffix(".tmp")
    tmp_path.write_text(json.dumps({"blocks": [block.model_dump() for block in blocks]}, ensure_ascii=False), encoding="utf-8")
    tmp_path.replace(path)


def _page_cache_path(key: str) -> Path:
    return Path(settings.storage_dir) / "cache" / "ocr_pages" / f"{key}.json"
