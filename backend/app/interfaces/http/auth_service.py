...
def oauth_configuration_status() -> dict[str, Any]:
    ...
    if settings.oauth_enabled and provider == "oidc":
        required = {
            "CHARTLENS_OAUTH_CLIENT_ID": settings.oauth_client_id,
            "CHARTLENS_OAUTH_AUTHORIZATION_URL": settings.oauth_authorization_url,
            "CHARTLENS_OAUTH_TOKEN_URL": settings.oauth_token_url,
            "CHARTLENS_OAUTH_USERINFO_URL": settings.oauth_userinfo_url,
        }
        missing = [key for key, value in required.items() if not value]

    warnings: list[str] = []
    if settings.oauth_enabled and settings.oauth_session_secret == "change-me-for-production":
        warnings.append("CHARTLENS_OAUTH_SESSION_SECRET is using the default development value")
...
def auth_status_from_request(request: Request) -> dict[str, Any]:
    ...
    session_auth = session_auth_status_from_request(request)
...
def model_auth_status() -> dict[str, Any]:
    ...
    try:
        from app.infrastructure.auth.chatgpt_token_store import chatgpt_token_cache_status, has_chatgpt_model_auth

        chatgpt_codex_configured = has_chatgpt_model_auth()
        token_status = chatgpt_token_cache_status()
    except Exception:
        chatgpt_codex_configured = False
        token_status = {
            "token_cache_exists": False,
            "token_cache_path": str(settings.chatgpt_token_cache_path),
            "updated_at": None,
            "expires_at": None,
            "user": None,
        }
...
def session_auth_status_from_request(request: Request) -> dict[str, Any]:
    if not settings.oauth_enabled:
        return {
            "enabled": False,
            "authenticated": True,
            "provider": "local",
            "user": {"sub": "local-dev", "email": "local-dev@localhost", "name": "Local Dev"},
            "issued_at": None,
            "expires_at": None,
            "cookie_name": settings.oauth_session_cookie,
        }
    ...
    expires_at = None
    if payload:
        issued_at = int(payload.get("iat", 0) or 0) or None
        expires_at = issued_at + settings.oauth_session_ttl_seconds if issued_at else None
        user = {
...
def session_payload_from_request(request: Request) -> dict[str, Any] | None:
    ...
    if not token:
        return None
    return verify_signed_payload(token, max_age_seconds=settings.oauth_session_ttl_seconds)
...
def build_authorization_redirect(next_path: str = "/") -> tuple[str, str]:
    ...
    if settings.oauth_provider == "chatgpt":
        from app.infrastructure.auth.chatgpt_oauth import begin_chatgpt_login
