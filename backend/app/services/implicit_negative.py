...
from app.schemas.pipeline import DocumentFragment, FieldExtractionResult
...
def apply_implicit_negative(
    ...
    *,
    ...
    unknown_terms = load_system_config().medical_dictionaries.unknown_terms + ["未详", "不清", "记不清"]
    if any(term in combined_text for term in unknown_terms):
        return result

    positive_terms = [term for term in terms_for_field(field.key) + field.synonyms if term]
