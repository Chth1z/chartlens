...
| `mock_general` (extraction, LLM-assisted) | `OpenAICompatibleChatProvider` -> DeepSeek v4-flash | 0.9861 (71/72) | 1.0 (67/67) | 1.0 (67/67) | 0.0 | 10 synthetic cases. On 2026-05-18 the `rule_pre_accepted` shortcut (E1-005) closed the `eval-mock-003 / age` LLM gap by routing high-confidence demographics rule hits past the LLM entirely; baseline rises from 0.9722 to 0.9861 (chosen run; across 5 cache-cleared runs the spread is 70-72/72 with `age` deterministically PASS in every run, residual variance is pre-existing LLM stochasticity on the `hospital` and `drinking_history` paraphrases). Token cost: 73,037 input / 19,743 output on the chosen run (the `eval-mock-009 / hospital` paraphrase miss replaces the prior `eval-mock-003 / age` echo as the single failure). Per-case input cost on demographics-group fields is reduced because `gender` / `age` / `hospital` (when rule confidence ≥ 0.95) no longer round-trip through the LLM at all; the small input-token rise versus the prior 51,037 baseline reflects DeepSeek v4-flash's response-length sensitivity to the v2 prompt across runs and the wider 10-case fixture set, not a per-call regression. |
...
- Outcome (partial, 2026-05-18): synonym widening for the eval-mock-008 challenge case. Added `血压偏高 / 血压增高 / 血压高 / BP高` to `hypertension_history.synonyms` and `嗜酒 / 喝酒 / 酗酒` to `drinking_history.synonyms`. Both pinned regression tests inverted from MISSING to PASS in the same commit. Baseline regenerated at `accuracy=1.0` (54/54).
- Outcome (final, 2026-05-18): the `rule_pre_accepted` shortcut is now wired in `_extract_document_evidence_first` (`backend/app/services/pipeline.py`). Before calling `evidence_provider.collect_evidence`, the pipeline partitions phase-1 fields into rule-pre-accepted candidates (group `semantic_strategy == "rule_shortcut"` AND `rule_shortcut_extract` returns confidence ≥ 0.95) and `llm_fields` (everything else). Pre-accepted candidates skip the LLM evidence-first chain entirely, are tagged with `acceptance_reason="rule_pre_accepted"` and `provenance.source="rule_shortcut"` / `provenance.skipped_llm=True` (with `decision_status="PASS"` so the export gate still admits them), and are merged back into `candidates_by_key` after the LLM stages so a stale LLM result cannot overwrite them. The `field_count` in trace.step calls now reflects the reduced LLM count. The `eval-mock-003 / age` LLM gap is closed: `_extract_age` returns `'72'` deterministically at confidence 0.96, the LLM never sees `age` in its `fields` argument, and the result lands as `normalized_code='72'` with `acceptance_reason='rule_pre_accepted'` in every run. LLM-assisted baseline rises from 0.9722 (70/72) to 0.9861 (71/72) on the chosen baseline run; across 5 cache-cleared runs the spread is 70-72/72 with the `age` field passing in every run, confirming the fix is deterministic and the residual variance is pre-existing LLM stochasticity on `hospital` / `drinking_history` paraphrases. New regression test `test_rule_shortcut_high_confidence_skips_llm_collect_evidence` in `test_evidence_first_extraction.py` builds an asserting fake provider that raises `RuntimeError` if `age` ever appears in `collect_evidence`'s `fields` argument; it is the contract ratchet so a future refactor cannot silently re-route `age` through the LLM. Backend tests 342 → 343.
