...
| `mock_general` (extraction, LLM-assisted) | `OpenAICompatibleChatProvider` -> DeepSeek v4-flash | 0.994 (158/159) | 1.0 | 1.0 | 0.0 | 19 synthetic cases. Last update 2026-05-19 by E1-001 v3.1 prompt rewrite + raised timeline rule-pattern confidences (155/159 → 158/159 best, 4 cache-cleared runs span 155-158/159 = 0.974-0.994). v3.1 closed eval-mock-007 chronic-disease implicit-negative variance under family-history mention; raising timeline confidences to ≥ 0.95 lets `onset_to_admission_time` and `admission_to_surgery_time` qualify for the rule_pre_accepted shortcut. Single residual failure is `transfer eval-mock-018` (llm_semantic field, LLM extraction variance). Token cost 249,967 input / 25,832 output on the committed run. Anchor: ROADMAP E1-001 v3.1. |
...
- Outcome (2026-05-19, partial): first incremental pass split the two oversized files. `adapters.py` (760 lines) is now an `adapters/` subpackage with `openai_responses.py`, `openai_compatible.py`, `anthropic.py`, `gemini.py` (each ≤ 253 lines) plus an `__init__.py` that re-exports the four provider classes and the names tests monkey-patch. `payloads.py` (691 lines) shed evidence-first helpers into `payloads_evidence_first.py` (380); the remaining `payloads.py` (335) re-exports them for backward compatibility. No behavior change: all previously-passing 343 backend tests still pass; rule baseline 0.9623 (153/159) unchanged; LLM baseline 0.9748 (155/159), well above the 0.94 floor; frontend tests and build pass; governance scan passes with no large-file warnings. The full router/credentials reorganization (`router.py`, `credentials.py`) remains future work tracked under E0-006-adjacent followups. Anchor: `PLAN.md` Done "E0-004 Split llm_provider adapters and payloads".
