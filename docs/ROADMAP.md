...
| --- | --- | --- | --- | --- | --- | --- |
| `mock_general` (extraction) | `ConservativeLocalProvider` (rule-only) | 1.0 (54/54) | 1.0 (54/54) | 1.0 (54/54) | 0.0 | 8 synthetic cases. Raised from 0.9630 to 1.0 on 2026-05-18 by E1-005 synonym widening (`hypertension_history` adds `血压偏高/血压增高/血压高/BP高`; `drinking_history` adds `嗜酒/喝酒/酗酒`). Rule-only baseline is the floor that any precision change must hold. |
| `mock_general` (extraction, LLM-assisted) | `OpenAICompatibleChatProvider` -> DeepSeek v4-flash | 0.9259 (50/54) | 1.0 (50/50) | 1.0 (50/50) | 0.0 | 8 synthetic cases, ~72k input tokens, ~19k output tokens. Established 2026-05-18 by E1-011 Phase 2. The 4 failing fields are all on `eval-mock-007` where the source says `既往史：无特殊` and the rule path correctly returns `0` for hypertension/diabetes/heart_disease/stroke via implicit-negative; the LLM returns `unknown` because the current evidence-first prompt does not teach it that section-complete negation in `既往史` covers all chronic-disease fields. Direct target for E1-001 (evidence-first prompt rewrite). The baseline is a real before/after reference: token cost is non-zero, accuracy is below rule-only by exactly 4 fields, and the failure pattern is a single root cause. |
